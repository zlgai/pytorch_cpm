...
def read_data_file(root_dir):
    ...
    image_nums_arr = np.array([float(os.path.split(s)[-1][2:-4]) for s in image_arr])
...
def read_mat_file(mode, root_dir, img_list):
    """
        ...
        Notice:
            ...
            from kpts guess center point & scale
        
        Returns:
           kpts     float[2000][14][3], float[3]对应 {float x; float y; float prob;}
           centers float[2000][2]
           scales float[2000]
    ...
    if mode == 'lsp':
        ...
        lms = mat_arr.transpose([2, 0, 1]) # shape=[2000,3,14]
        kpts = mat_arr.transpose([2, 1, 0]).tolist() # (({float x;float y;float prob})[14])[2000]
