...
class CocoDataset(Dataset):
    def __init__(self, csv_file, insize, mode='train', n_samples=None):
        # self.coco = coco
    ...
    def resize_data(self, img, ignore_mask, poses, shape):
        ...
        return resized_img, poses

    def random_resize_img(self, img, poses):
        h, w, _ = img.shape
        ...
        resized_img, resized_poses = self.resize_data(img, poses, shape)
        return resized_img, poses
    ...
    def random_crop_img(self, img, poses):
        h, w, _ = img.shape
        insize = self.insize
        joint_bboxes = self.get_pose_bboxes(poses)
        ...
        poses[:, :, :2] -= offset
        return crop_img.astype('uint8'), poses
    ...
    def augment_data(self, img, poses):
        aug_img = img.copy()
        aug_img, poses = self.random_resize_img(aug_img, poses)
        # aug_img, poses = self.random_rotate_img(aug_img, poses)
        aug_img, poses = self.random_crop_img(aug_img, poses)
        if np.random.randint(2):
            aug_img = self.distort_color(aug_img)
        # if np.random.randint(2):
        #     aug_img, poses = self.flip_img(aug_img, poses)

        return aug_img, poses
    ...
    def generate_heatmaps(self, img, poses, heatmap_sigma):
        ...
        for joint_index in range(self.num_class):
            heatmap = np.zeros(img.shape[:-1])
            for pose in poses:
                if 1: # pose[joint_index, 2] > 0:
    ...
    def parse_coco_annotation(self, annotations):
        ...
        '''将coco注释数据注释转换为姿势数组'''
        poses = np.zeros((0, self.num_class, 3), dtype=np.int32)

        for ann in annotations:
            ann_pose = np.array(ann['keypoints']).reshape(-1, 3)
            pose = np.zeros((1, self.num_class, 3), dtype=np.int32)
...
#         gt_pose = np.array(ann['keypoints']).reshape(-1, 3)
    ...
    def __getitem__(self, i):
        ...
        pts = self.landmarks_frame.iloc[idx, 0:-5].values.astype('float').reshape(-1, 2)

        if self.mode == 'eval':
            return img, img_id

        poses = self.parse_coco_annotation(annotations)
        img, poses = self.augment_data(img, poses)
        resized_img, resized_poses = self.resize_data(img, poses, shape=(self.insize, self.insize))
        heatmaps = self.generate_heatmaps(resized_img, resized_poses, params['heatmap_sigma'])

        resized_img = torch.tensor(self.preprocess(resized_img))
        heatmaps = torch.tensor(heatmaps)
        return resized_img, heatmaps