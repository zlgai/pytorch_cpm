...
class LSPDataset(Dataset):
    def __init__(self, root_dir, transform=None, phase_train=True,
        ...
        if lms is None or imagefiles is None or weight is None:
            ...
            image_nums_lsp = np.array([float(os.path.split(s)[-1][2:-4]) for s in image_lsp])
            image_nums_lspet = np.array([float(os.path.split(s)[-1][2:-4]) for s in image_lspet])
    ...
    def __getitem__(self, idx):
        """
            1. 获取图片集 imagefiles [num] ，点标注集 lms  shape=[num][2, num_class]
            2. resize lm
            3. lm => heatmap shape=[num_class, mw, mh]
            4. heatmap 计算背景 [1, mw, mh]
            5. 合并 [num_class+1, mw,mh]
            4. lm =>guess center point 
            7. center point => generate centermap shape=[mw,mh]
            8. 
        """
        if not self.phase_train:
            idx += self.num_train
        image = imageio.imread(self.imagefiles[idx])
